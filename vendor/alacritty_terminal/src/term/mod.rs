...
use crate::kitty::{KittyContext, KittyScene, KittyState, KittyUpdates};
...
pub struct Term<T> {
    ...
    /// Kitty placements are independent of text cells, unlike Sixel fragments.
    kitty: KittyState,
    kitty_apc: Vec<u8>,
    kitty_apc_overflow: bool,
    graphics_frozen: bool,
...
impl<T> Term<T> {
    ...
    pub fn new<D: Dimensions>(config: Config, dimensions: &D, event_proxy: T) -> Term<T> {
        ...
        Term {
            ...
            kitty_apc: Vec::new(),
            kitty_apc_overflow: false,
            graphics_frozen: false,
    ...
    /// Mark the complete terminal as needing redraw.
    #[inline]
        ...
        T: EventListener,
    {
        ...
        self.kitty.resize(self.kitty_context(), 0);
        self.kitty
            .resize_inactive(self.kitty_grid_context(&self.inactive_grid), 0);
    ...
    /// Pixel geometry used by graphic source and destination rectangles.
    pub fn graphics_cell_size(&self) -> (u16, u16) {
        (
            (self.graphics.cell_width as u16).max(1),
            (self.graphics.cell_height as u16).max(1),
        )
    }

    fn kitty_context(&self) -> KittyContext {
        self.kitty_grid_context(&self.grid)
    }

    fn kitty_grid_context(&self, grid: &Grid<Cell>) -> KittyContext {
    ...
    pub fn kitty_scene(&self, display_offset: usize) -> KittyScene {
        self.kitty
            .scene(&self.grid, display_offset, self.kitty_context())
    }

    pub fn kitty_take_updates(&mut self) -> KittyUpdates {
        self.kitty.take_updates()
    }

    pub fn kitty_forget_resource(&mut self, id: u64) -> bool {
        self.kitty.forget_resource(id)
    }

    pub fn kitty_tick(&mut self, now: Instant) -> bool {
        if self.graphics_frozen {
            return false;
        }
        let changed = self.kitty.tick(now);
        if changed {
            self.mark_fully_damaged();
        }
        changed
    ...
    pub fn kitty_next_deadline(&self) -> Option<Instant> {
        if self.graphics_frozen {
            None
        } else {
            self.kitty.deadline()
    ...
    pub fn set_kitty_file_transfers(&mut self, allowed: bool) {
        self.kitty.set_file_transfers(allowed);
    }

    /// Freeze the terminal's final checkpoint after its PTY session exits.
    pub fn freeze_graphics(&mut self) {
        self.kitty_tick(Instant::now());
        self.graphics_frozen = true;
    }
    ...
    pub fn resize<S: Dimensions>(&mut self, size: S) {
        ...
        // Graphics geometry may change while the grid geometry stays the same.
        let old_cell_size = self.graphics_cell_size();
        self.graphics.resize(&size);
        if old_cell_size != self.graphics_cell_size() {
            self.mark_fully_damaged();
        }
        ...
        let inactive_min_delta = cmp::min(
            0,
            num_lines as i32 - self.inactive_grid.cursor.point.line.0 - 1,
        );
        let inactive_delta = (num_lines as i32 - old_lines as i32)
            .max(inactive_min_delta)
            .min(self.inactive_grid.history_size() as i32);
        ...
        self.kitty.resize(self.kitty_context(), delta);
        self.kitty
            .resize_inactive(self.kitty_grid_context(&self.inactive_grid), inactive_delta);
    }
    ...
    pub fn swap_alt(&mut self) {
        ...
        self.kitty
            .switch_screen(self.mode.contains(TermMode::ALT_SCREEN));
        self.kitty.resize(self.kitty_context(), 0);
        self.selection = None;
        self.mark_fully_damaged();
    }
        ...
        T: EventListener,
    {
        ...
        self.kitty.clear_visible(self.kitty_context());
        self.mark_fully_damaged();
    }
...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn reset_state(&mut self) {
        ...
        self.grid.reset();
        self.kitty.reset();
        self.kitty_apc.clear();
        self.kitty_apc_overflow = false;
    ...
    fn apc_put(&mut self, byte: u8) {
        if self.kitty_apc.first().is_some_and(|first| *first != b'G') {
            return;
        }
        if self.kitty_apc.len() < 8192 {
            self.kitty_apc.push(byte);
        } else {
            self.kitty_apc_overflow = true;
        }
    }

    fn apc_end(&mut self, completed: bool) {
        if !completed {
            if self.kitty_apc.first() == Some(&b'G') {
                self.kitty.abort_transfer();
            }
            self.kitty_apc.clear();
            return;
        }
        ...
        if self.kitty_apc_overflow {
            // A truncated payload must never accidentally decode successfully.
            self.kitty_apc.push(0);
        }
        let result = self.kitty.handle_apc(
            &self.kitty_apc,
            self.kitty_context(),
            &self.grid,
            Instant::now(),
        ...
        if let Some((row, column)) = result.cursor {
            self.grid.cursor.point = Point::new(
                Line(row.clamp(0, self.bottommost_line().0)),
                Column(column.min(self.columns().saturating_sub(1))),
            );
            self.grid.cursor.input_needs_wrap = false;
        }
        if result.changed || result.cursor.is_some() {
            self.mark_fully_damaged();
        }
    }
}
