...
/// Maximum RGBA bytes retained for graphics in one parser batch or session.
pub const MAX_GRAPHIC_BYTES: usize = 16 * 1024 * 1024;

/// Maximum graphics retained in one parser batch or session.
pub const MAX_GRAPHIC_ASSETS: usize = 128;
...
impl GraphicId {
    /// Return this graphic's session-unique numeric identifier.
    #[inline]
    pub fn get(self) -> u64 {
        self.0
    }
...
pub struct Graphics {
    ...
    /// RGBA bytes represented by [`Self::pending`].
    pub pending_rgba_bytes: usize,
...
impl Graphics {
    ...
    pub fn take_queues(&mut self) -> Option<UpdateQueues> {
        ...
        self.pending_rgba_bytes = 0;
        Some(UpdateQueues { pending, remove_queue, clear_subregions })
    }
...
pub fn insert_graphic<L: EventListener>(
    ...
    let rgba_bytes = match graphic
        .width
        .checked_mul(graphic.height)
        .and_then(|pixels| pixels.checked_mul(4))
    {
        Some(bytes) if bytes <= MAX_GRAPHIC_BYTES => bytes,
        _ => return,
    };

    if term.graphics.pending.len() >= MAX_GRAPHIC_ASSETS
        || term.graphics.pending_rgba_bytes > MAX_GRAPHIC_BYTES - rgba_bytes
    {
        return;
    }
    ...
    term.graphics.pending_rgba_bytes += rgba_bytes;
...
#[test]
fn ignores_graphics_without_pixel_geometry() {
    let dimensions = TestDimensions { columns: 2, lines: 2, cell_width: 0., cell_height: 0. };
    let mut term = Term::new(crate::term::Config::default(), &dimensions, crate::event::VoidListener);

    insert_graphic(&mut term, test_graphic(), None);

    assert!(term.graphics_take_queues().is_none());
}

#[test]
fn resize_updates_graphics_for_pixel_only_changes() {
    let dimensions = TestDimensions { columns: 2, lines: 2, cell_width: 8., cell_height: 16. };
    let mut term = Term::new(crate::term::Config::default(), &dimensions, crate::event::VoidListener);
    let resized = TestDimensions { columns: 2, lines: 2, cell_width: 10., cell_height: 20. };

    term.resize(resized);
    insert_graphic(&mut term, test_graphic(), None);

    let graphics = term.grid()[Line(0)][Column(0)].graphics().unwrap();
    assert_eq!(graphics[0].texture.cell_height, 20);
}

#[test]
fn limits_pending_graphic_count_before_queue_drain() {
    let dimensions = TestDimensions { columns: 2, lines: 2, cell_width: 8., cell_height: 16. };
    let mut term = Term::new(crate::term::Config::default(), &dimensions, crate::event::VoidListener);

    for _ in 0..=MAX_GRAPHIC_ASSETS {
        insert_graphic(&mut term, test_graphic(), None);
    }

    assert_eq!(term.graphics.pending.len(), MAX_GRAPHIC_ASSETS);
}

#[test]
fn limits_pending_rgba_bytes_before_queue_drain() {
    let dimensions = TestDimensions { columns: 2, lines: 2, cell_width: 8., cell_height: 16. };
    let mut term = Term::new(crate::term::Config::default(), &dimensions, crate::event::VoidListener);
    let graphic = || GraphicData {
        id: GraphicId(0),
        width: 1024,
        height: MAX_GRAPHIC_BYTES / (1024 * 4),
        color_type: ColorType::Rgba,
        pixels: vec![255; MAX_GRAPHIC_BYTES],
        is_opaque: true,
    };

    insert_graphic(&mut term, graphic(), None);
    insert_graphic(&mut term, graphic(), None);

    assert_eq!(term.graphics.pending.len(), 1);
    assert_eq!(term.graphics.pending_rgba_bytes, MAX_GRAPHIC_BYTES);
}
