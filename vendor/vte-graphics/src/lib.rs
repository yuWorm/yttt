...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    #[inline(always)]
    fn advance_apc<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match byte {
            0x18 | 0x1A => {
...
pub trait Perform {
    ...
    /// Pass a byte in an application program command.
    fn apc_put(&mut self, _byte: u8) {}

    /// Complete an APC; false means cancellation rather than an ST terminator.
    fn apc_end(&mut self, _completed: bool) {}
...
mod apc_tests {
    ...
    impl Perform for Terminal {
        ...
        fn apc_end(&mut self, completed: bool) {
            if completed {
                self.completed.push(core::mem::take(&mut self.pending));
            } else {
                self.cancelled += 1;
                self.pending.clear();
            }
    ...
    #[test]
    fn split_apc_is_only_delivered_after_the_complete_terminator() {
        let mut parser = Parser::new();
        let mut terminal = Terminal::default();
        for byte in b"before\x1b_Ga=q,i=1;AAAA\x1b" {
            parser.advance(&mut terminal, &[*byte]);
            assert!(terminal.completed.is_empty());
        }
        parser.advance(&mut terminal, b"\\after");
        assert_eq!(terminal.completed, [b"Ga=q,i=1;AAAA".to_vec()]);
        assert_eq!(terminal.text, "beforeafter");
    }

    #[test]
    fn interrupted_apc_does_not_commit_or_consume_the_following_escape() {
        let mut parser = Parser::new();
        let mut terminal = Terminal::default();
        parser.advance(&mut terminal, b"\x1b_Gdiscard\x1b[31mtext\x1b_Gcancel\x18");
        parser.advance(&mut terminal, b"\x1b_Gkeep\x1b\\");
        assert_eq!(terminal.completed, [b"Gkeep".to_vec()]);
        assert_eq!(terminal.cancelled, 2);
