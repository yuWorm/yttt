...
use crate::mouse::{
    encode_modifiers, mouse_button_report, pixel_to_cell, pixels_to_scroll_lines, scroll_report,
...
impl TerminalView {
        ...
        R: Read + Send + 'static,
    {
        ...
        // Create event proxy for alacritty. Terminal-generated responses such as
        // device attributes must be written back immediately, not delayed until a
        // later render pass.
        let event_proxy = GpuiEventProxy::new(event_tx).with_pty_writer(stdin_writer.clone());
    ...
    fn paste_clipboard_to_terminal(&self) -> bool {
        let Ok(text) = Clipboard::new().and_then(|mut clipboard| clipboard.paste()) else {
            return false;
        };
    ...
    fn point_for_position(&self, position: Point<Pixels>) -> Option<AlacPoint> {
        ...
        let point = pixel_to_cell(position, origin, viewport.cell_width, viewport.cell_height);
        let row = point
            .line
            .0
            .clamp(0, viewport.rows.saturating_sub(1) as i32);
        let col = point.column.0.min(viewport.cols.saturating_sub(1));

        Some(AlacPoint::new(Line(row - display_offset), Column(col)))
    }

    fn modifiers_for_mouse_report(modifiers: Modifiers) -> u8 {
        encode_modifiers(modifiers.shift, modifiers.alt, modifiers.control)
    }
    ...
    fn is_copy_keystroke(keystroke: &Keystroke) -> bool {
        ...
        #[cfg(target_os = "macos")]
        {
            keystroke.modifiers.platform && !keystroke.modifiers.alt && !keystroke.modifiers.control
        }
    ...
    fn on_mouse_down(
        ...
        let modifiers = Self::modifiers_for_mouse_report(event.modifiers);
    ...
    fn on_scroll(
        ...
        let Some(viewport) = (*self.viewport.lock()).clone() else {
            return;
        };
        let Some(point) = self.point_for_position(event.position) else {
            return;
            ...
            .child(
                canvas(
                    ...
                    move |bounds, _, window, cx| {
                        ...
                        *viewport.lock() = Some(TerminalViewport {
                            bounds,
