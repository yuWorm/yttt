...
//! ```ignore
//! use gpui::px;
//! use yttt_terminal::{ColorPalette, TerminalRenderer};
//!
...
/// renderer.paint(bounds, padding, true, &term, window, cx);
/// ```
///
/// # Performance
///
...
impl TerminalRenderer {
    ...
    /// ```
    /// use gpui::px;
    /// use yttt_terminal::render::TerminalRenderer;
    ...
    pub fn paint(
        ...
        let selection_background = self.palette.selection_background();
        ...
        for line_idx in 0..num_lines {
            ...
            let cells: Vec<(usize, Cell)> = (0..num_cols)
                .map(|col_idx| {
                    ...
                    let point = AlacPoint::new(grid_line, col);
        ...
        if show_scrollbar {
            self.paint_scrollbar(bounds, padding, term, window);
        }
    }
    ...
    fn paint_scrollbar(
        ...
        let right_inset = px(3.0);
        let track_bounds = Bounds {
            origin: Point {
                x: bounds.origin.x + bounds.size.width - right_inset - track_width,
                y: bounds.origin.y + padding.top,
            },
            size: Size {
                width: track_width,
        ...
        let thumb_y = track_bounds.origin.y + track_height * metrics.thumb_top_fraction;
