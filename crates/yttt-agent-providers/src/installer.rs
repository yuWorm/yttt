...
use crate::{OMP_EXTENSION_SOURCE, OPENCODE_PLUGIN_SOURCE, PI_EXTENSION_SOURCE};
...
fn atomic_write(path: &Path, bytes: &[u8]) -> io::Result<()> {
    ...
    let parent = path
        .parent()
        .ok_or_else(|| io::Error::other("hook path has no parent"))?;
...
pub fn install_managed_hooks_at(config_dir: &Path, home: &Path) -> io::Result<()> {
...
mod tests {
    ...
    fn malformed_user_hook_config_is_not_overwritten() {
        ...
        assert!(install_managed_hooks_at(&config_dir, &home).is_err());
