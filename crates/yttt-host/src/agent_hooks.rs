...
impl HostAgentHookRuntime {
    pub fn start() -> io::Result<Arc<Self>> {
        ...
        let accept_thread = thread::Builder::new()
            ...
            .spawn(move || accept_loop(listener, thread_secret, thread_events, thread_shutdown))?;
        Ok(Arc::new(Self {
            endpoint,
            secret,
            events,
    ...
    pub fn environment(&self, scope: AgentHookScope) -> AgentHookEnvironment {
        ...
        AgentHookEnvironment {
            variables: BTreeMap::from([
                ...
                (ENVIRONMENT_VARIABLES[2].to_string(), scope),
            ]),
        }
    }
...
fn accept_loop(
    ...
    while !shutdown.load(Ordering::Acquire) {
        match listener.accept() {
            Ok((stream, _)) => handle_connection(stream, &secret, &events),
...
fn handle_connection(
    ...
    let request =
        read_http_request(&mut stream).and_then(|request| decode_request(request, secret));
    let status = match request {
        Ok(request) => {
            let _ = events.send(ServerEvent::AgentHook(request));
            204
        }
...
fn decode_request(
    request: HttpRequest,
    secret: &[u8; 32],
) -> Result<AgentHookEvent, HttpRequestError> {
    ...
    let payload_json = serde_json::to_vec(&payload).map_err(|_| HttpRequestError::Invalid)?;
    Ok(AgentHookEvent {
        scope,
...
mod tests {
    ...
    fn encoded_scope(scope: &AgentHookScope) -> String {
        URL_SAFE_NO_PAD.encode(serde_json::to_vec(scope).unwrap())
    }

    #[test]
    fn hook_scope_token_cannot_be_reused_for_another_pane() {
        let secret = [7_u8; 32];
        let first = encoded_scope(&AgentHookScope {
            project_id: "project".to_string(),
            tab_id: "tab".to_string(),
            pane_id: "first".to_string(),
            generation: 1,
        });
        let second = encoded_scope(&AgentHookScope {
            project_id: "project".to_string(),
            tab_id: "tab".to_string(),
    ...
    #[test]
    fn hook_decoder_preserves_authenticated_json_payload_bytes() {
        let secret = [9_u8; 32];
        let scope = AgentHookScope {
            project_id: "project".to_string(),
            tab_id: "tab".to_string(),
        ...
        let encoded = encoded_scope(&scope);
        let request = HttpRequest {
            path: "/hook/omp".to_string(),
            headers: BTreeMap::from([
                (SCOPE_HEADER.to_string(), encoded.clone()),
                (TOKEN_HEADER.to_string(), scope_token(&secret, &encoded)),
            ]),
            body: br#"{"event":"turn-finished","payload":{"result":"ok"}}"#.to_vec(),
        };

        let event = decode_request(request, &secret).unwrap();
        assert_eq!(event.scope, scope);
        assert_eq!(event.source, "omp");
        assert_eq!(event.event, "turn-finished");
        assert_eq!(event.payload_json, br#"{"result":"ok"}"#);
    }
}
