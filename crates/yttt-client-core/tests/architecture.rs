...
#[test]
fn client_core_production_dependencies_exclude_concrete_transports() {
    let manifest = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/Cargo.toml"));
    let forbidden = forbidden_production_deps(manifest);
    assert!(
        forbidden.is_empty(),
        "yttt-client-core production dependencies must not include concrete transports: {forbidden:?}"
    );
}
